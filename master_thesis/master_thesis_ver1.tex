...
本論文では,大域探索と局所探索を調節可能なBat Algorithmと個体間距離に基づく動的変化を考慮したNiching schemeを組み合わせることによる多峰性最適化問題における，複数解探索アルゴリズムを構築し，その手法の有効性を検証する．
従来の多点探索アルゴリズムは一つの最適解に収束する傾向にあるが，実問題の複雑さを多峰性で表した時，一つの最適解だけでなく複数の最適解及び局所解を探索することは，有力な解候補を選択肢として複数保持しておくという意味で非常に重要である．複数解探索を行う手法は，進化的アルゴリズムのGAに基づき，未探索領域へ解を生成するための機構であるNiching schemeを組み合わせた{\it Niching method}に関する研究が数多くなされてきたが，いずれの手法においても密接した解を除き，探索空間内にランダムに解生成するため，乱数に強く依存する傾向にある．そこで本研究では，多点探索アルゴリズムの中でも大域と局所の探索バランスに優れたBat Algortihmを採用し，各個体の探索領域を分割させるNiching scheme を用いることで最適解だけでなく，局所解も同時に探索可能な3つの複数解探索手法を提案した．(i)未探索領域へ解を生成する機構としてNovelty Searchを導入した，Novelty Search-based Bat Algorithm (NSBA); (ii)探索空間の大きさと最適解数から算出されるNiche Radiusを用いることで，予め各個体の探索領域を決定し，その探索領域内の最良個体から遠ざかる方向へ移動し，個体同士が同じ解に留まらず，分散させるNiche Radius-based Bat Algorithm (NRBA); (iii)個体の分布密度が高いNiche Radius内で，評価値の低い個体をその領域から移動させるNRBAを拡張したBat Algorithm with Dynamic Niche Radius (DNRBA)．
従来手法と提案手法の性能を比較するため，最適解と局所解の数が異なるパターンの多峰性関数を用いてシミュレーション実験を行った結果，従来手法のBAは一つの最適解に収束していたが，提案手法では全最適解及び局所解を探索することができ，従来に対する変更点が有効であることを示した．
